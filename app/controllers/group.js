...
//   // Class serach string - serves as class list div toggle
//   $scope.searchText = new Object();
//   $scope.searchText.txt = '';
    
//   // Get all class objects
// 	// $ref.base.child('classcodes').once('value', function(snapshot){
//  //    $scope.allClassesObj = snapshot.val();
// 	// 	for(obj in snapshot.val()) {
// 	// 		var cls = snapshot.val()[obj];
// 	// 		$scope.allClasses.push(cls);
//  //    }
//  //    $scope.classLoadWaiting = false;
//  //  });
...
//   // Toggle classes list div when clicked
//   $scope.closeClassList = function(strg) {
//     strg.txt = '';
//   }
...
//   		if($scope.user.classes == undefined ||
//   		   $scope.user.classes[clickedClass.code] == undefined) {
//         $firebase($ref.base.child("users/" + $ref.netid).child("/classes/" + clickedClass.code)).$set(
//           {name:clickedClass.name, code:clickedClass.code, prof:clickedClass.prof});
//         $firebase($ref.base.child("classcodes/" + clickedClass.code + "/members/" + $ref.netid)).$set(
//           {member : true}
//         );
//       }
// 		  else
...
//   // Modal to appear when class remove icon is clicked


//   // Modal to appear when adding group icon is clicked
//   $scope.openAddGroupModal = function(clickedClass) {

//     var members = $scope.allClassesObj[clickedClass.code]['members'];
//     $scope.classMembers = [];

//     var getUserInfo = function(index){
//       if(index < Object.keys(members).length) {
//         var key = Object.keys(members)[index];
//         $ref.base.child('users/' + key).on('value', function(snapshot){
//           var obj = snapshot.val();
//           obj.netid = key;
//           $scope.classMembers.push(obj);
//           getUserInfo(index+1);
//         });
//       } else {
//         var modalInstance = $modal.open({
...
//   var GroupModalInstanceCtrl = function ($scope, $modalInstance, msg, members, classCode) {
//   	$scope.msg = msg;
//   	$scope.newGroupMembers = [];
//     $scope.classMembers = members;
...
//       console.log($scope.newGroupMembers);
//       for(i = 0; i < $scope.newGroupMembers.length; i++) {
//         $firebase($ref.base.child("users/" + $ref.netid + "/classes/" + classCode + "/groups/" 
  //+ $scope.groupName)).$set({name:$scope.groupName, type:'open'});
//         $firebase($ref.base.child("users/" + $ref.netid + "/classes/" + classCode + 
  //"/groups/" + $scope.groupName + "/members/" + $scope.newGroupMembers[i].netid)).$set(
//           {exists:true});
//       }
//   		$modalInstance.close();
//   	}
...
//     var transferMember = function(from, to, member) {
//       console.log("Transfering " + member.netid);
//       to.push(member);
//       var ind = from.indexOf(member);
//       from.splice(ind, 1);
//     }
